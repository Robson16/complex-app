...
User.prototype.validate = function() {
    return new Promise(async (resolve, reject) => {
        ...
        if (this.data.username.length > 30) this.errors.push("Username cannot exceed 30 caracters.");
    
        if (!validator.isEmail(this.data.email)) this.errors.push("You must provide a valide email address.");
    
        if (this.data.password == "") this.errors.push("You must provide a valide a password.");
        if (this.data.password.length > 0 && this.data.password.length < 12) this.errors.push("Password must be at least 12 caracters");
        if (this.data.password.length > 50) this.errors.push("Password cannot exceed 50 caracters.");
